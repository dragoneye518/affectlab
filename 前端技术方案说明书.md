...
### 3.4 AI 接入与容错
封装 `utils/api.js`：
- **请求**：`wx.request` 调用 DeepSeek `/chat/completions`。
- **Prompt**：预设 `SYSTEM_INSTRUCTION`，要求返回 JSON 格式的三种风格文案 (TOXIC/EMO/GLITCH)。
- **Fallback**：若网络超时、Key 失效或 API 报错，立即返回本地预设的“伪 AI”结果，保证用户体验不中断。
